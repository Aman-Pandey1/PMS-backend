...
export async function login(req, res) {
	try {
		...
		return res.json({ user: { id: String(user._id), email: user.email, fullName: user.fullName, role: user.role, companyId: user.companyId ? String(user.companyId) : null }, token });
	} catch (error) {
...
export async function me(req, res) {
	...
	return res.json({ id: String(user._id), email: user.email, fullName: user.fullName, role: user.role, companyId: user.companyId ? String(user.companyId) : null });
}
...
export async function updateMe(req, res) {
	...
	try {
		...
		return res.json({ id: String(updated._id), email: updated.email, fullName: updated.fullName, role: updated.role, companyId: updated.companyId ? String(updated.companyId) : null });
	} catch (e) {
