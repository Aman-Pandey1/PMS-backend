...
export async function getUserSalary(req: Request, res: Response) {
	const { id } = req.params;
	const items = await Salary.find({ userId: id }).sort({ effectiveFrom: -1 });
	res.json({ items });
}

export async function setUserSalary(req: Request, res: Response) {
	const { id } = req.params;
	const { designation, baseSalary, securityAmount, effectiveFrom } = req.body as any;
...
export async function companyPayrollSummary(req: Request, res: Response) {
